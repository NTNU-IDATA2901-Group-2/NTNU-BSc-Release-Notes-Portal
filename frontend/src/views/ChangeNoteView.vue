...
<template>
  <main class="flex flex-col items-center px-4 mb-20">
    ...
    <div class="md:hidden flex w-full mt-4 justify-end gap-2">
      ...
      <Button disabled v-if="isEditing" variant="outline">Save
        <Save />
      </Button>
    </div>
    ...
    <div v-if="!isPending && !isFetching && !isError && changeNote"
      ...
      <div class="flex flex-col gap-4 w-full">
        <div class="flex flex-row items-center justify-between w-full">
          <div class="flex items-center gap-4">
            ...
            <Input v-if="isEditing" class="w-full" v-model="changeNote.reference" placeholder="Reference" />
          ...
          <div class="flex gap-4">
            ...
            <Button class="hidden md:flex" disabled v-if="isEditing" variant="outline">Save
              <Save />
            </Button>
          </div>
        </div>
        ...
        <Textarea v-if="isEditing" placeholder="Description of change" class="w-full" v-model="changeNote.description" />
        ...
        <div v-if="isEditing" class="flex flex-wrap justify-between gap-4">
          <div class="flex flex-col gap-1">
            ...
            <TagSelect mode="product" :selected-id="changeNote.product?.id"
          ...
          <div class="flex flex-col gap-1">
            ...
            <TagSelect mode="feature" :selected-id="changeNote.feature?.id"
              v-model="selectedFeature" />
          </div>
          <div class="flex flex-col gap-1">
            <h4 class="text-md">Customer</h4>
            <TagSelect mode="customer" :selected-id="changeNote.customer?.id"
      ...
      <div class="flex flex-col w-full text-xl gap-10">
        <div>
          ...
          <Textarea placeholder="Developer notes" v-if="isEditing" class="w-full" v-model="changeNote.developerNotes" />
        </div>
        <div>
          <h3 class="text-lg">Upgrade Notes</h3>
          <p v-if="!isEditing" class="text-sm">{{ changeNote.upgradeNotes }}</p>
          <Textarea placeholder="Upgrade notes" v-if="isEditing" class="w-full" v-model="changeNote.upgradeNotes" />
        </div>
      </div>
    </div>
  </main>
